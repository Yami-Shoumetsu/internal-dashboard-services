...
public class RestUtils {
    ...
    public static Link createHateoasLinks(Class<?> resource, String method, String rel, Map<String, Object> queryParams, Object... params) {
        UriBuilder uriBuilder = createUriBuilder(resource, method, queryParams);
     ...
     *            map of query parameter names and its value
     * @param params
     *            map of URI template parameter names and values that will be
     *            used to resolve variables in the urls
     * @return a HATEOAS {@link Link}
     */
    public static Link createHateoasLinks(Class<?> resource, String method, String rel, Map<String, Object> queryParams, Map<String, Object> params) {
        UriBuilder uriBuilder = createUriBuilder(resource, method, queryParams);
        URI uri = uriBuilder.buildFromMap(params);
        return new Link(uri.toString(), rel);
    }

    private static UriBuilder createUriBuilder(Class<?> resource, String method, Map<String, Object> queryParams) {
        UriBuilder uriBuilder;
        if (StringUtils.isEmpty(method)) {
            uriBuilder = UriBuilder.fromUri(ServletUriComponentsBuilder.fromCurrentServletMapping().toUriString()).path(resource);
        } else {
            uriBuilder = UriBuilder.fromUri(ServletUriComponentsBuilder.fromCurrentServletMapping().toUriString())
                                   .path(resource)
    ...
    public static URI createNewResourceUri(Class<?> resource, String method, Object... params) {
        return UriBuilder.fromUri(ServletUriComponentsBuilder.fromCurrentServletMapping().toUriString())
                         .path(resource)
                         .path(resource, method)
                         .build(params);
    }
